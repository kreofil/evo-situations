...
//------------------------------------------------------------------------------
// Использование процедурно-параметрического полиморфизма для заполнения карты.
// Обобщенная функция проверяющая входной символ и формирующая
// необходимое значение клетки
_Bool CreateCell<Cell* c>(char ch, EWorld* eWorld, int i, int j) {
    printf("[%d, %d]: Incorrect symbol '%c'\n", i, j, eWorld->textMap[i][j]);
    return 0;
}
// Обработчик специализации, осуществляющий формирование пустой клетки
_Bool CreateCell<Cell.empty* c>(char ch, EWorld* eWorld, int i, int j) {
    if(ch == ' ') {
...
// Обработчик специализации, осуществляющий формирование границы поля
_Bool CreateCell<Cell.locked* c>(char ch, EWorld* eWorld, int i, int j) {
    if(ch == '#') {
...
// Обработчик специализации, осуществляющий формирование живой клетки
_Bool CreateCell<Cell.filled* c>(char ch, EWorld* eWorld, int i, int j) {
    if(ch == '*') {
...
void CreateMap(EWorld* eWorld) {
    ...
    // Определение количества альтернатив, используемых для проверки
    int variantNumbers = get_spec_size(Cell);
    ...
    for(int i = 0; i < eWorld->mapLines; ++i) {
        ...
        for(int j = 0; j < width-1; ++j) {
            ...
            // Формирование значения клетки на карте
            for(int k = 1; k < variantNumbers; ++k) {
                Cell* c = get_spec_ptr(Cell, k);
                if(CreateCell<c>(ch, eWorld, i, j)) {break;}
            }
        }
        width = nextWidth;
    }
...
//------------------------------------------------------------------------------
// Полиморфное формирование значения клетки в зависимости от числа соседей и
// своего состояния. Обобщенная функция пустая
void NewCellState<Cell* c>(int count, EWorld* eWorld, int i, int j) {}
// Обработчик специализации для пустой клетки
void NewCellState<Cell.empty* c>(int count, EWorld* eWorld, int i, int j) {
...
// Обработчик специализации для живой клетки
void NewCellState<Cell.filled* c>(int count, EWorld* eWorld, int i, int j) {
...
void NewStateOfCell(EWorld* eWorld, int i, int j) {
    ...
    NewCellState<pCurrentCell>(count, eWorld, i, j);
}
