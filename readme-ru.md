...
В каталоге планируется размещение контейнеров, аналогичных по функциям контейнерным классам языка C++. Отсутствие в языке C шаблонов не позволяет полностью покрыть возможности, предоставляемые C++. Однако процедурно-параметрический полиморфизм позволяет сделать контейнеры с аналогичной подменов типов значений. В настоящий момент реализован аналог класса `std::vector`.
