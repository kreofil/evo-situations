...
figure_circle c = {.tail.r=13};
// Конструктор, обеспечивающий инициализацию тега переменной r
void  __attribute__ ((constructor(401))) init_figure_circle_c() {
    printf("%s\n", __FUNCTION__);   // тестовый вывод конструктора
    init_figure_circle(&c);
}
...
int main() {
    ...
    // Круг
    figure *p_c = (figure*)malloc(sizeof(figure_circle));
    init_figure_circle((figure_circle*)p_c);
    ((figure_circle*)p_c)->tail.r = 13;
    print_figure(p_c);
    free(p_c);
    ...
    multimethod_figure(p_f, p_c);
    multimethod_figure(p_t, p_c);
    multimethod_figure(p_c, p_f);
    multimethod_figure(p_c, p_t);
    multimethod_figure(p_c, p_c);
