...
//BEGIN
	...
	In::Int(tag);
	if(tag == 1) {
		pfr = new O2M_UNNM_pfr(O2M_UNNM_pfr::O2M_INIT_rect);
		rectangle::Input(&r);
		pfr->O2M_SPEC_rect->x = r.x;
		pfr->O2M_SPEC_rect->y = r.y;
		pf = pfr;
	} else
	if(tag == 2) {
		pft = new O2M_UNNM_pft(O2M_UNNM_pft::O2M_INIT_trian);
		triangle::Input(&t);
		pft->O2M_SPEC_trian->a = t.a;
		pft->O2M_SPEC_trian->b = t.b;
		pft->O2M_SPEC_trian->c = t.c;
		pf = pft;
	} else {
		pf = 0;
	};
